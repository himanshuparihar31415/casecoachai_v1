...
export default function Dashboard({ user }: { user: AuthUser | null }) {
  ...
  useEffect(() => {
    Promise.all([
      api.get<TrendPoint[]>('/analytics/trends'),
      api.get<Overview>('/analytics/overview'),
      api.get<{ history: HistoryItem[] }>('/analytics/history?limit=5'),
    ]).then(([trends, ov, hist]) => {
      setTrendData(trends);
      setOverview(ov);
  ...
  const metrics = overview
    ? [
        { label: 'Structure', value: overview.competencies.structure },
        { label: 'Math', value: overview.competencies.math },
        { label: 'Business Judgment', value: overview.competencies.businessJudgment },
        { label: 'Communication', value: overview.competencies.communication },
      ]
    : [];
      ...
      <section className="flex flex-col md:flex-row justify-between items-end gap-8">
        <div className="max-w-2xl">
          ...
          <h1 className="power-gap-h1 text-primary mb-6">
            Welcome back, <br />
            <span className="text-on-tertiary-container">{firstName}!</span>
          </h1>
          <p className="font-body text-secondary text-lg max-w-md">
            {overview && overview.totalSessions > 0
              ? `You've completed ${overview.totalSessions} session${overview.totalSessions !== 1 ? 's' : ''} with an average score of ${overview.avgOverallScore}%. Keep it up!`
              : 'Start your first session to begin tracking your performance.'}
          </p>
        </div>
        <Link
          to="/config"
      ...
      {loading ? (
        ...
        <>
          ...
          {metrics.length > 0 ? (
            ...
            <div className="grid grid-cols-1 md:grid-cols-4 gap-8">
              {['Structure', 'Math', 'Business Judgment', 'Communication'].map((label) => (
                <div key={label} className="bg-surface-container-lowest p-8 rounded-xl flex flex-col justify-between border border-outline-variant/15">
                  <span className="label-blueprint">{label}</span>
                  <div className="mt-8">
                    <div className="text-4xl font-headline font-extrabold text-primary/20">—</div>
                    <div className="h-1 w-full bg-surface-container mt-4 rounded-full" />
                  </div>
                </div>
              ))}
            </div>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-12">
            ...
            <div className="lg:col-span-2 bg-surface-container-low rounded-xl p-10">
              ...
              {trendData.length > 0 ? (
                ...
                <div className="h-64 flex items-center justify-center text-secondary text-sm italic">
                  Complete your first session to see trend data
                </div>
              )}
              <div className="flex justify-between mt-4 label-blueprint border-t border-outline-variant/10 pt-4">
                <span>Session 01</span>
                <span>Session {Math.max(trendData.length, 1)}</span>
              </div>
            </div>

            {/* Strategic Focus (weaknesses) */}
            <div className="bg-surface-container-low rounded-xl p-10">
              <h3 className="font-headline text-2xl font-bold text-primary mb-8">Competency Scores</h3>
              {overview && overview.totalSessions > 0 ? (
                <div className="space-y-10">
                  {[
                    { label: 'Creativity', value: overview.competencies.creativity },
                    { label: 'Hypothesis', value: overview.competencies.hypothesis },
                    { label: 'Structure', value: overview.competencies.structure },
                  ].map((item) => (
                    <div key={item.label}>
                ...
                <p className="text-secondary text-sm italic">No data yet</p>
              )}
              <Link
                to="/library"
            ...
            <div className="overflow-x-auto">
              <table className="w-full text-left">
                ...
                <tbody className="font-body text-sm text-primary divide-y divide-outline-variant/5">
                  {history.length === 0 ? (
                    <tr>
                      <td colSpan={5} className="px-8 py-12 text-center text-secondary italic">
                        No completed sessions yet. <Link to="/config" className="text-primary font-bold underline">Start your first case</Link>.
                      </td>
                    </tr>
                  ) : (
                    history.map((item) => (
                      <tr key={item.sessionId} className="hover:bg-surface-container transition-colors group">
                        <td className="px-8 py-6 font-bold">{item.caseTitle}</td>
                        <td className="px-8 py-6 text-secondary capitalize">{item.caseType.replace('_', ' ')}</td>
                        ...
                        <td className="px-8 py-6 font-headline font-extrabold text-on-tertiary-container">
